...
exports.createRequest = async (req, res) => {
    try {
        ...
        if (existing) {
            return res.status(400).json({ message: "You already have a pending request with this donor" });
        }
...
// ✅ This was missing — frontend calls /api/requests/my
...
// ================= GET INCOMING REQUESTS (as donor, for dashboard notification) =================
// ✅ This was missing — frontend calls /api/requests/incoming
...
exports.rateDonor = async (req, res) => {
    try {
        ...
        donor.rating = donor.rating
            ? parseFloat(((donor.rating + rating) / 2).toFixed(1))
            : rating;
        await donor.save();
